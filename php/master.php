...
/* INWARD MASTER */
	...
	if($action=='AddPurchaseDetailsToDB'){
		...
		$flag=false;
			...
			$selProdid="SELECT `purchase_id` FROM `purchase_register` where `purchase_date`=".$data->purchaseTm;
			$resProdid=mysql_query($selProdid);
			$rowProdid = mysql_fetch_array($resProdid,MYSQL_BOTH);

		if($resupdtStock){
			$flag=true;
		}
		$obj->resinsProd=$resinsProd;
		$obj->purchaseId=$rowProdid['purchase_id'];
		 if($flag==true){
			$obj->status=true;
		 }
		 else{
...
/* 				$newOstk=floatval($rowoutward['stock_avail'])-floatval($data->quantity);
	...
	if($action=='makeInwardPayment'){
		$data = json_decode(file_get_contents("php://input"));
			/* PayFlag is true if pay amount is 0 */
			if($data->payFlag==true){
				$insAccounts1="INSERT INTO `account_register`(`acc_client_id`, `purchaseSaleId`, `acc_type`, `credit_debit`, `acc_amount`, `acc_date`, `acc_month`, `acc_year`) VALUES (".$data->supplierid.", ".$data->purchaseId.", 'inward', 'credit','".$data->finalAmt."','".$data->purchaseTm."','".$data->purchaseMnt."','".$data->purchaseYr."')";
				...
				$insAccounts2="INSERT INTO `account_register`(`acc_client_id`, `purchaseSaleId`, `acc_type`, `credit_debit`, `acc_amount`, `acc_date`, `acc_month`, `acc_year`, `acc_particulars`) VALUES (".$data->supplierid.", ".$data->purchaseId.", 'inward', 'debit','".$data->payAmount."','".$data->purchaseTm."','".$data->purchaseMnt."','".$data->purchaseYr."','".$data->payParticulars."')";
...
/* Accounts */
	...
	if($action == 'fetchAccDetails'){
		...
		if($count>0){
			$cnt=0;
			while($row = mysql_fetch_array( $resAccClients )) {
				$selProdClients="SELECT *  FROM `purchase_register` WHERE  purchase_id=".$row['purchaseSaleId']." and purchase_date='".$row['acc_date']."'";
				$resProdClients=mysql_query($selProdClients);
				$rowProdClients = mysql_fetch_array($resProdClients,MYSQL_BOTH);
				
				$selProdBagClients="SELECT *  FROM `purchase_bag_register` WHERE  purchasebag_id=".$row['purchaseSaleId']." and purchase_date='".$row['acc_date']."'";
				$resProdBagClients=mysql_query($selProdBagClients);
				$rowProdBagClients = mysql_fetch_array($resProdBagClients,MYSQL_BOTH);

				if(mysql_num_rows($resProdClients)>0){
					$tmpRes[$cnt]->prodDets=$rowProdClients;
				}
				else{
					$tmpRes[$cnt]->prodDets=$rowProdBagClients;
				}
	...
	if($action=='allPendingOrders'){
		$selAccClients="SELECT * FROM `sales_register`,`client_master` WHERE sales_register.sale_status='open' and sales_register.client_id=client_master.client_id order by `order_date`";
		$resAccClients=mysql_query($selAccClients);
		$count = mysql_num_rows($resAccClients);
		if($count>0){
			$cnt=0;
